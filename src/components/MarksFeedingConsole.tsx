...
export default function MarksFeedingConsole() {
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white border border-slate-200/80 rounded-2xl p-6 shadow-sm text-left">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 border-b border-slate-150 pb-5">
          <div className="flex items-center gap-3">
            <div className="h-10 w-10 bg-indigo-50 border border-indigo-100 rounded-xl flex items-center justify-center text-indigo-600 shrink-0 shadow-sm">
              <BookOpen className="h-5 w-5" />
            </div>
            <div>
              <h3 className="text-base font-extrabold text-slate-800 tracking-tight">
                Academic Marks & Marksheet Console
              </h3>
              <p className="text-xs text-slate-400 font-medium mt-0.5">
                Feed student scores, view class analytics, and generate official CBSE A4 Marksheets.
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2.5 shrink-0">
            <button
              ...
              className="py-2 px-3.5 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 border border-emerald-200 rounded-xl text-xs font-bold flex items-center gap-1.5 transition-all cursor-pointer shadow-sm"
            >
            ...
            <button
              ...
              className="py-2 px-3.5 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 border border-indigo-200 rounded-xl text-xs font-bold flex items-center gap-1.5 transition-all cursor-pointer shadow-sm"
            >
              <Printer className="h-4 w-4" />
              Bulk Print Class Marksheets
            </button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 mt-5">
          <div>
            ...
            <select
              ...
              className="w-full text-xs font-bold py-2 px-3 border border-slate-200 rounded-xl outline-none bg-slate-50 focus:bg-white focus:border-indigo-600 text-slate-700 cursor-pointer shadow-sm"
            >
            ...
            {!isCustomSubjectMode ? (
              ...
              <div className="flex gap-1.5">
                <input
                  ...
                  className="w-full text-xs font-bold py-2 px-3 border border-slate-200 rounded-xl outline-none bg-slate-50 focus:bg-white focus:border-indigo-600 text-slate-700"
                />
                <button
                  type="button"
                  onClick={() => setIsCustomSubjectMode(false)}
                  className="px-2 bg-slate-100 hover:bg-slate-200 text-slate-600 border border-slate-200 rounded-xl text-[10px] font-bold"
                >
                  Cancel
                </button>
              </div>
          ...
          <div>
            ...
            <input
              ...
              className="w-full text-xs font-bold py-2 px-3 border border-slate-200 rounded-xl outline-none bg-slate-50 focus:bg-white focus:border-indigo-600 text-slate-700 disabled:opacity-60 disabled:cursor-not-allowed shadow-sm"
            />
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-left">
        ...
        <div className="bg-white border border-slate-200/80 p-4 rounded-2xl shadow-sm flex items-center gap-3">
          ...
          <div>
            ...
            <h4 className="text-xs font-black text-slate-800 mt-0.5 truncate max-w-[120px]" title={topScorerName}>
              {topScorerName !== "--" ? `${highestScore} pts (${topScorerName})` : "--"}
            </h4>
          </div>
        </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white p-4 rounded-2xl border border-slate-200/80 shadow-sm text-left">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="pl-8 pr-3 py-1.5 text-xs font-semibold border border-slate-200 rounded-xl outline-none bg-slate-50 focus:bg-white focus:border-indigo-600 w-full sm:w-64"
          />
        </div>
      </div>

      {successMsg && (
        <div className="flex items-center gap-2 bg-emerald-50 text-emerald-800 p-4 rounded-2xl border border-emerald-200 text-xs font-bold shadow-sm text-left">
      ...
      {errorMsg && (
        <div className="flex items-center gap-2 bg-rose-50 text-rose-800 p-4 rounded-2xl border border-rose-200 text-xs font-bold shadow-sm text-left">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden text-left">
        <div className="p-4 bg-slate-50/80 border-b border-slate-200 flex items-center justify-between">
          <h3 className="text-xs font-black uppercase tracking-wider text-slate-700">
            Student Marks Entry Table ({filteredStudents.length} Students)
          </h3>
          <button
                    ...
                    return (
                      <tr key={student.id} className={`hover:bg-slate-50/70 transition-colors ${hasAnyInvalid ? "bg-rose-50/30" : ""}`}>
                        ...
                        {splitComponents.map((comp: any, cIdx: number) => {
                          ...
                          return (
                            <td key={cIdx} className="py-3.5 px-2 text-center">
                              <input
                                type="number"
                                step="0.5"
                                min="0"
