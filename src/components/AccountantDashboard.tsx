...
export default function AccountantDashboard() {
  ...
  // Math Calculations for Accountant
  const totalCollections = receipts.reduce((sum, r) => sum + r.amount, 0);
  const totalOutstanding = dueItems.filter((d) => d.status === "UNPAID").reduce((sum, s) => sum + s.amount, 0);
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white border border-slate-200/80 p-5 rounded-xl shadow-sm">
          <span className="text-[9px] font-black uppercase text-slate-400 block mb-1">Total Daily Collection</span>
          <h3 className="text-lg font-black text-green-600 tracking-tight">
            Rs. {totalCollections.toLocaleString("en-IN")}
          </h3>
          <p className="text-[9px] text-slate-400 font-semibold mt-0.5">Calculated from {receipts.length} receipts</p>
        </div>
        <div className="bg-white border border-slate-200/80 p-5 rounded-xl shadow-sm">
          <span className="text-[9px] font-black uppercase text-slate-400 block mb-1">Total Outstanding Dues</span>
          <h3 className="text-lg font-black text-rose-600 tracking-tight">
            Rs. {totalOutstanding.toLocaleString("en-IN")}
          </h3>
          <p className="text-[9px] text-slate-400 font-semibold mt-0.5">{unpaidStudents.length} students pending</p>
        </div>
        <div className="bg-white border border-slate-200/80 p-5 rounded-xl shadow-sm">
          <span className="text-[9px] font-black uppercase text-slate-400 block mb-1">Active Students</span>
          <h3 className="text-lg font-black text-slate-700 tracking-tight">
            {students.length}
          </h3>
          <p className="text-[9px] text-slate-400 font-semibold mt-0.5">Across classes 9 & 10</p>
        </div>
      </div>
